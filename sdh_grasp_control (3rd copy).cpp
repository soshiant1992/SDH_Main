...
//        cout<<"sensoraftercontact_distal"<<Pstrong[1]+Pstrong[3]+Pstrong[5]<<endl;
    ...
    if(grasp_type=="cube" && gripit && (Pstrong[3]>threshold[3]*0)
            &&(Pstrong[5]>threshold[5]*0)){
        gripit=false;
        backoff2();
        cout<<"wabalabadobdob"<<endl;
    }
    if(gripit && (Pstrong[1]>20)
            && (Pstrong[3]>20)
            &&(Pstrong[5]>20)){
        gripit=false;
        backoff2();
    }
//    if(gripit && Pstrong[0]+Pstrong[2]+Pstrong[4]>10){
...
void domainloop() {
    ...
    if(gripit){
        ...
        {
//              pro_controller(0.2, (error[0]+error[4]+error[2])/3, d_theta[i]) ; /// d_theta should be positive for proximal
              pro_controller(0.1, error[i], d_theta[i]) ; /// d_theta should be positive for proximal

//            if(!justproximal)pro_controller(0.2, (error[3]+error[1]+error[5])/3, d_theta[i+1]) ; // then d_theta should be positive for distal
